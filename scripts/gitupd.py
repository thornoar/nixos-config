...
if (subprocess.call(
        ['git', '-C', args.target, 'status'],
        stderr = subprocess.STDOUT,
        stdout = open(os.devnull, 'w')
    ) == 0):
    branch = run_command(["git", "-C", args.target, "rev-parse", "--abbrev-ref", "HEAD"])
    os.system("git -C " + args.target + " remote update")
    status = run_command(["git", "-C", args.target, "status"])
    ...
    if "branch is up to date" in status:
        ...
        if "working tree clean" in status:
            ...
            print("local changes:")
            os.system("git -C " + args.target + " status -s")
            print("\033[34m> Pushing local changes...\033[0m") #]]
            os.system("git -C " + args.target + " add . && git -C " + args.target + " commit -m \"" + args.message +"\" && git -C " + args.target + " push")
        ...
        os.system("git -C " + args.target + " fetch && git -C " + args.target + " pull")
else:
